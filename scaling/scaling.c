...
void processTransformed(float* magPrev, float* phasePrev, float* magNew,
                        float* phaseNew, float* magOutPrev, float* phaseOutPrev,
                        float* magOutNew, float* phaseOutNew, double phaseScaleAmount) {
    ...
    float expectedDPhase = 0;
    for (int i = 0; i < WINDOW_SIZE / 2; i++) {
        float dPhase = wrapPhase(phaseNew[i] - phasePrev[i]);
        expectedDPhase += 2 * M_PI * HOP_LENGTH / WINDOW_SIZE;
    ...
    for (int i = 0; i < WINDOW_SIZE / 2; i++) {
        ...
        else if (phaseOutPrev[i] == 0 && magOutPrev[i] == 0) {
            newPhase = synthBinDeviations[i];        }
        else {
            float phaseRemainder = synthBinDeviations[i] * 2 * M_PI * HOP_LENGTH / WINDOW_SIZE;
            newPhase = phaseOutPrev[i] + phaseRemainder + (i * 2 * M_PI * HOP_LENGTH / WINDOW_SIZE);
        }
        phaseOutNew[i] = newPhase;
        magOutNew[i] = synthMags[i];

        if (i != 0) {
            phaseOutNew[WINDOW_SIZE - 1] = newPhase;
            magOutNew[WINDOW_SIZE  - 1] = synthMags[i];
        }
    }
}
